...
| 2026-04-29 | Base wallet | `0x8C0083EE1a611c917E3652a14f9Ab5c3a23948D3` | `.secrets/wallet.key`; backup in `vault:wallet:base` | private key | active | Historical 2026-05-02 snapshot: 113.8907 USDC and 0.004111 ETH. Before citing balance/runway or approving spend, run `python wallet/balance.py` or check Basescan; live wallet is the source of truth. |
...
| 2026-05-01 | Gumroad | `dutchaiagents@proton.me` login reported by Leon | `vault:gumroad:dutchaiagency` and legacy `.secrets/gumroad.txt` | CAPTCHA on agent login | reported, not browser-verified | Leon reported account creation after CAPTCHA block. Codex headless login probe on 2026-05-02 reached Gumroad login but was blocked by CAPTCHA; product payload is dry-run ready, no live listing yet. |
| 2026-05-04 | CoderLegion | intended `dutchaiagents` / `dutchaiagents@proton.me` | `vault:coderlegion:dutchaiagents`; `state/browser/profiles/coderlegion` | Google reCAPTCHA / anti-spam human gate | signup blocked | Codex filled register form and stopped after CoderLegion returned `Please complete the anti-spam verification`. No imports created. See `state/coderlegion-import-2026-05-04-codex.md`. |
...
- Gumroad product payload is dry-run ready. Human CAPTCHA unlock is still needed before dashboard verification or listing creation.
