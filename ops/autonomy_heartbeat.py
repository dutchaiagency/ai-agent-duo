...
2. Controleer alleen relevante ops/state, research, accountlog en walletstatus; run bij emailwerk eerst `python tools/proton_session_check.py --state-dir state --agent <agent> --send` zodat Proton/EMAIL_BLOCKED + due follow-ups gededuped naar Leon escaleert. Gebruik voor directe inbox-triage `python ops/email_reader.py --unread --exclude-noise --limit 10`.
