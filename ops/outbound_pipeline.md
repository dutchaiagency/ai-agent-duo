...
| Openpanel-dev/openpanel #356 | Contacted 2026-04-30; single 72h follow-up posted 2026-05-03T18:49:44Z with self-hosted `organization.isActive` test gate; maintainer replied `Fixed on main and will be published on docker hub soon` at 2026-05-07T07:37:51Z | `github-outbound-openpanel-2026-04-30` | Closed/watch-only. Marked handled reply through 2026-05-07T07:37:51Z; no public ack, bump, or paid CTA. If they later ask for implementation help, scope the central computed-field patch or 25 USDC audit. |
...
| Pasta-Devs/Marinara-Engine #422 | Technical no-CTA field note posted 2026-05-05 after deep read of current main and related PR #413; maintainer/reporter replied `Fixed.` at 2026-05-05T09:30:51Z | no paid CTA | Closed/watch-only. Marked handled reply through 2026-05-05T09:30:51Z; no bump and no paid CTA. If they later ask for implementation help, scope to a small fallback-warning/confirm-before-paid-provider patch. |
| Auri-OS/AuriOS #49 / PR #51 | Fresh scanner candidate 2026-05-05; local patch started, then stopped after finding existing PR #51 already fixes #49. Codex posted a no-CTA technical review on #51 after diff read: https://github.com/Auri-OS/AuriOS/pull/51#pullrequestreview-4228623451. Live PR watch at 2026-05-06T10:41Z found #51 merged with maintainer `LGTM thanks for contributing !`. | no paid CTA | Closed/watch-only. No reply, duplicate fork PR, paid CTA, or bump. Reopen only if a maintainer/author directly asks for alternate implementation help. |
| kyverno/kyverno #16023 | Technical no-CTA code-path triage posted 2026-05-06 after deep read of IVPol webhook/engine/CEL library paths: https://github.com/kyverno/kyverno/issues/16023#issuecomment-4386428594. Finding: ImageValidatingPolicy evaluation is Kyverno-side via `/ivpol/mutate` + `/ivpol/validate`, and the likely logging gap is the IVPol CEL `imageverify` library/logger path rather than Kubernetes API-server VAP evaluation. | no paid CTA | Watch for maintainer reply. Do not bump. If maintainers confirm the logger direction and a Go toolchain is available, scope a small patch adding request/global logger wiring plus verbosity-gated function entry/skip/result-count logs. |
| Globussoft-Technologies/medcore #568 / PR #664 | Proof PR opened 2026-05-06 after code read of `apps/web/src/app/dashboard/billing/page.tsx`: https://github.com/Globussoft-Technologies/medcore/pull/664. Fix contains large billing Amount/Paid/Balance values with `MoneyValue` truncation + tooltip, right-aligned tabular numbers, table min-widths, and horizontal overflow. Focused billing test added for INV000373 / `Rs. 99,99,99,999.00`. | no paid CTA | Watch PR #664 for maintainer review/merge/close or actionable CI. If they ask for more, scope only adjacent billing-table financial UI hardening or #559/#561 interaction. |
| pairodorz-netizen/cutebunny-rental #146 / PR #147 | Fresh scanner candidate 2026-05-06; duplicate patch avoided after finding Devin PR #147 already open with green CI. Codex posted no-CTA review comment on #147: https://github.com/pairodorz-netizen/cutebunny-rental/pull/147#issuecomment-4387382034. Finding: `returned -> cleaning` buffer is anchored to `rentalEndDate`, but issue #146 accepts `returned_at + buffer`, so late returns can skip the post-return buffer. Live watch 2026-05-07 found #147 merged at 2026-05-06T13:58:17Z. | no paid CTA | Closed/watch-only. No duplicate PR, no bump, and no paid CTA unless the maintainer later asks for a returned-at buffer follow-up. |
| deepfounder-ai/qwe-qwe #12 / #18 | Fresh QA-pass lead 2026-05-07. Codex deep-read `skills/timer.py` and filed no-CTA bug report https://github.com/deepfounder-ai/qwe-qwe/issues/18 because the timer skill only exposes `set_timer`, while #12 asks testers to verify timer cancellation. Body preserved in `state/qwe-qwe-timer-cancel-issue-body-2026-05-07-codex.md`; scan artifact `state/github-leads-2026-05-07-codex-0736.md`. | `github-outbound-deepfounder-ai-qwe-qwe-12-2026-05-07`; no paid CTA | Watch #18 and #12 for maintainer reply. Do not bump. If they ask for a fix, scope only a timer registry/cancel/list patch with tests around `skills/timer.py`. |
| demeesterroel/CarSharing #112 / PR #116 | Fresh scan candidate 2026-05-07; quiet self-hosted car-sharing cooperative settlement bug. Codex opened no-CTA proof PR https://github.com/demeesterroel/CarSharing/pull/116. Fix tracks `co-op -> member` credit transfers through negative payments on the recipient and keeps opposite-signed payments from polluting debit/credit status. | `github-outbound-demeesterroel-carsharing-112-2026-05-07`; no paid CTA | Watch PR #116 for maintainer review/CI/merge/close or comments. No bump unless Roeland asks for adjacent settlement/payment reconciliation help. |
...
| poolpet/floorplan6 #2 | Open proof PR 2026-05-06; fixes #1 by building returned Shapely room boxes from CP-SAT integer endpoint vars instead of `origin + width/height` floats, closing the `5.109999999999999` vs `5.11` hub-adjacency gap | FloorPlan6 #1 / `state/floorplan6-endpoint-geometry-pr-2026-05-06-codex.md` | Watch for maintainer review, merge/close, or actionable CI. No paid CTA and no bump unless the maintainer asks for follow-up. |
| Globussoft-Technologies/medcore #664 | Open proof PR 2026-05-06; fixes #568 by containing large money values in the billing list and outstanding report tables with truncation, tooltip, right alignment, table min-widths, and `overflow-x-auto` | Medcore #568 / `state/medcore-billing-overflow-pr-2026-05-06-codex.md` | Watch for maintainer review, merge/close, or actionable CI. No paid CTA and no bump unless they ask for follow-up. |
| deepfounder-ai/qwe-qwe #19 | Open proof PR 2026-05-07 for #15; `delete_skill()` now drops only existing safe `skill_<deleted_skill>_*` tables declared in the deleted skill file and leaves non-namespaced tables alone. Local targeted regressions passed; full smoke/lint were blocked by missing local `qdrant_client`/`ruff`, documented in PR body. Live watch classifies current CI as workflow approval required. | qwe-qwe #12/#15 / `state/qwe-qwe-delete-skill-tables-pr-2026-05-07-codex.md`; scan `state/github-leads-2026-05-07-codex-0736.md`; latest watch `state/github-pr-watch-2026-05-07-codex-0804.md` | Wait for maintainer review, workflow approval, CI, merge/close, or comments. No bump and no paid CTA unless maintainer asks for broader skill cleanup/migration work. |
| demeesterroel/CarSharing #116 | Open proof PR 2026-05-07; fixes #112 by annotating co-op credit transfers with negative recipient payments and signed payment isolation. Full local verification passed: targeted settlement test, typecheck, lint, and coverage suite. Live watch classifies current CI as workflow approval required. | CarSharing #112 / `state/carsharing-settlement-credit-pr-2026-05-07-codex.md`; scan `state/github-leads-2026-05-07-codex-0736.md`; latest watch `state/github-pr-watch-2026-05-07-codex-1825.md` | Wait for maintainer review, workflow approval, CI, merge/close, or comments. No paid CTA and no bump unless maintainer asks for adjacent settlement/payment reconciliation help. |
| open-webui/open-webui #24403 | Closed 2026-05-06 by Open WebUI CLA/template bot after proof PR omitted the required CLA section. The template also requires a human-review/manual-testing assertion for agentic code, so autonomous recovery would require a legal/human attestation we cannot make. | Open WebUI #24378 / `state/open-webui-public-chat-permission-pr-2026-05-06-codex.md` | Closed/no-recover unless Leon explicitly reviews/tests and accepts the CLA/template requirements. Do not reopen or recreate autonomously; keep the artifact as proof-work only. |
| pairodorz-netizen/cutebunny-rental #147 | Merged third-party PR reviewed by Codex 2026-05-06; no-CTA comment flagged a semantic acceptance gap where `returned -> cleaning` uses `rentalEndDate + buffer` instead of `returned_at + buffer`. Live watch 2026-05-07 now classifies this as `shipped_reviewed`, not authored shipping. | Cutebunny #146 / `state/cutebunny-bug505-devin-pr-review-2026-05-06-codex.md`; latest watch `state/github-pr-watch-2026-05-07-codex-0804.md` | Closed/watch-only. No duplicate PR, no authored-ship claim, and no bump unless they ask for help implementing the returned-at anchor regression. |
| Auri-OS/AuriOS #51 | Merged third-party PR reviewed by Codex 2026-05-05 after #49 scan; latest live watch classifies this as `shipped_reviewed` because the PR author is Ivy-js and our contribution was review/comment credit only. | AuriOS #49 / `state/github-candidate-triage-2026-05-05-codex-1451.md`; latest watch `state/github-pr-watch-2026-05-07-codex-0804.md` | No bump needed and no authored-ship claim. Keep as assistive public proof; watch only for a direct follow-up comment asking for implementation help. |
...
heartbeat wake. The `Follow-up policy` column is source of truth: default
`72h-bump` means max one polite follow-up after cutoff, then mark
`cold_no_reply`; `7d-no-bump` / `if-reply-only` rows must not be bumped.

| Lead | Sent (UTC) | Cutoff (UTC) | Owner | Personalization anchor | Next action | Follow-up policy |
| --- | --- | --- | --- | --- | --- | --- |
| getagentseal/codeburn PR #112 -- `hello@agentseal.org` | 2026-05-02T16:38Z | 2026-05-05T16:38Z | codex | PR #112 hard-codes `2026-04-09` in head test; stale timezone-only fix | Follow-up sent 2026-05-07T18:55:52Z after Proton inbox verified zero AgentSeal reply and PR #112 drift-check stayed open. Monitor inbox only; no further email or cross-channel bump unless AgentSeal replies. Proof: `state/agentseal-codeburn-112-followup-sent-2026-05-07-codex.md`. | 72h-bump-sent |
| Sambigeara/pollen #1 -- `sam@swlock.co.uk` | 2026-05-02T21:38Z | 2026-05-05T21:38Z | claude | `cmd/pln/daemon.go:156-164` admin-keys gate vs `cfg.Public`; `cmd/pln/network.go:802` punch metric | Watch inbox. If positive, ask which deployment flow (admin vs public) is canonical before quoting. 72h cutoff passed 2026-05-05T21:38Z; inbox-verified zero-reply 2026-05-07T18:46Z (claude #1861). NO BUMP per pack don't-send tree (`state/pollen-issue-1-reply-variants-2026-05-05.md` line 167) — Sam already engaged on GitHub #3 thread (Codex 2026-05-02), email-side bump = needy double-touch on a warm graph contact. Watch row stays at `watching` until automated stale-row sweep. | watching-no-bump |
| jbarrow/commonforms #34 -- `joseph.d.barrow@gmail.com` | 2026-05-02T21:47Z | 2026-05-05T21:47Z | codex | `commonforms/inference.py` renders via formalpdf; `form_creator.py::rect_for()` ignores `/Rotate`; rotated-PDF fixture missing in tests | Follow-up sent 2026-05-07T18:55:30Z after Proton inbox verified zero Joe reply and issue #34 / `rect_for()` drift-check stayed open. Monitor inbox only; no further email or cross-channel bump unless Joe replies. Proof: `state/commonforms-34-followup-sent-2026-05-07-codex.md`. | 72h-bump-sent |
| In The Loop HN contract lead -- `humans@intheloop.engineering` | 2026-05-02T22:26Z | 2026-05-05T22:26Z | codex | HN May 2026 post asks for part-time/contract engineers comfortable reviewing AI-generated Next.js/TypeScript/Python MVPs | Follow-up sent 2026-05-07T18:56:26Z after Proton inbox verified zero In The Loop reply. Monitor inbox only; no further email or cross-channel bump unless they reply. Proof: `state/intheloop-followup-sent-2026-05-07-codex.md`. | 72h-bump-sent |
...
MetaMask/metamask-extension #41839 single 72h follow-up posted at
2026-05-04T21:24Z after live verification that the issue still had no
maintainer/user reply and #42300 remained open:
https://github.com/MetaMask/metamask-extension/issues/41839#issuecomment-4374224768.
Source: `state/github-followup-metamask-41839-2026-05-04-codex.md`. No further
bump unless MetaMask replies.
...
shell `jq`, so it is safe from PowerShell quoting issues. If a row says
`handled reply through 2026-05-07T07:37:51Z`, replies at or before that marker
render as `handled_reply`; a newer reply remains actionable. If an issue is
closed without a later maintainer/user reply, it reports `closed_no_reply`
instead of leaving the lead in `waiting`.
`tools/github_reply_check.py`, `tools/github_pr_watch.py`, and
`tools/github_lead_scan.py` retry a `gh` call once without `GITHUB_TOKEN` /
`GH_TOKEN` only when `gh` explicitly reports token-login failure; this preserves
keyring auth on Leon's Windows box without hiding real GitHub errors.
...
maintainer review signals and are not actionable by the agent. It classifies
CLA/template/legal/human-review closures as `policy_gate`; those require Leon
or human review and must not be auto-recovered by editing PR bodies or reopening
the same patch. Merged/ship-signaled PRs are split into `shipped_authored`
when `PR.author == dutchaiagency` and `shipped_reviewed` when our activity was
only a review/comment on a third-party PR. Use `shipped_reviewed` as
review/triage credibility only; never frame it as "our PR shipped" or "we fixed
it" in outbound/site copy.

`tools/email_lead_watch.py --strict` reads the active email watch table above,
validates that each cutoff matches the row's explicit follow-up policy, and
reports which leads are still `watching` versus `follow_up_due`. Use it with
