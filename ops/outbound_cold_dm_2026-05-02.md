...
- **LOCK DEFAULT-ON / HARDENED 2026-05-03 20:05Z (codex)**: every live `--execute` takes a 600s `recipient:<email>` lock plus a 24h exact recipient+subject+body dedupe lock under `state/locks/` before Proton send. `--lock <topic>` only adds an extra workflow topic lock; it never replaces recipient/body dedupe. Proton invalid/missing-signature errors are treated as ambiguous send status: inspect Sent mail before retrying; no automatic resend.
